...
async function handleLogin(event) {
  ...
  if (!isSupabaseConfigured) {
    showMessage(
      "Supabase no está configurado. Esta es una versión demo.",
      "info"
    );
    return;
  }
...
async function submitQuestion(event) {
  ...
  if (!isSupabaseConfigured) {
    showMessage(
      "¡Gracias por tu pregunta! (Modo demo - Supabase no configurado)",
      "success"
    );
...
async function submitAttendeeForm(event) {
  ...
  if (!isSupabaseConfigured) {
    showMessage(
      "¡Gracias por tu información! (Modo demo - Supabase no configurado)",
      "success"
    );
    event.target.reset();
    return;
